...
import { useMonthlyProfitQuery } from '../redux/features/MonthlyProfitApi/monthlyProfitApi';

const MonthlyProfitRecord = () => {
  const { data, isLoading, isError } = useMonthlyProfitQuery();
  ...
  return (
    <>
      <h1 className='d-flex justify-content-center my-4 gradient_text'>Monthly Profit All Years Record</h1>
        ...
        {!isLoading &&
          ...
          Object.keys(yearlyData).map((year) => (
            <div key={year} className='yearly_record_card border rounded-2 my-1 form_div'>
              <Link
                to={`/monthly-profit-record-detail/${year}`}
...
export default MonthlyProfitRecord;
