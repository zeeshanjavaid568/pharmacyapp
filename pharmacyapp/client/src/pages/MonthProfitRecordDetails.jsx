...
import { useDeleteMonthlyProfitMutation, useMonthlyProfitQuery } from '../redux/features/MonthlyProfitApi/monthlyProfitApi';

const MonthProfitRecordDetails = () => {
    const location = useLocation();
    const { records } = location.state || { records: [] }; // Retrieve data passed via state

    const [searchProductName, setSearchProductName] = useState('');
    const [searchDate, setSearchDate] = useState('');
    const [deleteBuyerProduct] = useDeleteMonthlyProfitMutation(); // Hook for delete mutation
    const { refetch } = useMonthlyProfitQuery();
    ...
    const filteredRecords = records.filter((record) => {
        const matchesProductName = record.monthly_profit
            ?.toString()
            .toLowerCase()
                ...
                >
                    <thead style={{ position: 'sticky', top: '0', zIndex: '1000' }}>
                        <tr>
                            ...
                            <th style={{ backgroundColor: '#f44336', color: 'white' }}>Monthly Profit</th>
...
export default MonthProfitRecordDetails;
