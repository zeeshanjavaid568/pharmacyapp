...
class Dues {
  ...
  static async createGiveDues(data) {
    const {  name, single_piece_price, total_piece, given_dues, taken_dues, price, date } = data;
    const query = 'INSERT INTO dues ( name, single_piece_price, total_piece, given_dues, taken_dues, price, date) VALUES (?, ?, ?, ?, ?, ?, ?)';
    const [result] = await db.query(query, [ name, single_piece_price, total_piece, given_dues, taken_dues, price, date]);
